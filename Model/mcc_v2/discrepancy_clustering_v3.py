# Turtle notice marks the places where the experiment result may be improved
...
class DiscrepancyClustering(object):
    def __init__(self, instances, metric="euclidean", priority="recall", balance_towards_recall=1, \
        verbose=False):
        """
        self.pos_instances is a list that contains all the positive training samples
        self.neg_instances is a list that contains all the negative training samples
        ...
        self.num_pos_instances = len(self.pos_instances)
        self.num_neg_instances = len(self.neg_instances)
    ...
    def _reset_instances(self):
        # reset the clustering of all instances to none (unassigned)
    ...
    def _create_distance_mat(self):
        ...
        # split the distance matrix into two parts: negative matrix and positive matrix
        
        # will be (k, N+1) array for k pos instances & N centroids; the last col is index in all instances
        pos_dist_mat = []  
        # will be (k, N+1) array for k neg instances & N centroids; the last col is index in all instances
        neg_dist_mat = []  
    ...
    def _assign_positive_samples(self, pos_dist_mat, neg_dist_mat):
        # for each center, find the minimum distance between this center and any negative sample
        # Turtle notice: below is an alternative option to try for better result 

        # neg_thres = [max(pos_dist_mat[:, i]) for i in range(self.num_clusters)]
        ...
        for i in range(self.num_clusters):
            ...
            for n in index_in_pos:
                n = int(n)
                # update assigned/unassigned index record
                ...
                # # in v3, add the ins to the closest cluster
                # self.clusters[i].add_instance(self.pos_instances[n], float(pos_dist_mat[n, i]))
                
                # update instance record
                self.pos_instances[n].cluster.append(i)
                self.pos_instances[n].distances.append(float(pos_dist_mat[n, i]))

        for i in range(self.num_pos_instances):
            # after above step, if one ins belongs to only one cluster, then assign it to this cluster
            if len(self.pos_instances[i].cluster) == 1: 
                target_cluster = self.pos_instances[i].cluster[0]
                target_dist = self.pos_instances[i].distances[0]
                self.clusters[target_cluster].add_instance(self.pos_instances[i], target_dist)

            # after above step, if one ins belongs to multiple clusters, then assign it to the nearest one
            if len(self.pos_instances[i].cluster) > 1:
                dist_list = self.pos_instances[i].distances
                target_dist = min(dist_list)
                nearest_ind = dist_list.index(min(dist_list))
                target_cluster = self.pos_instances[i].cluster[nearest_ind]
                self.clusters[target_cluster].add_instance(self.pos_instances[i], target_dist)
    ...
    def _assign_false_positives(self, pos_dist_mat, neg_dist_mat):
        ...
        # this double-for-loop can be optimized in _assign_positive_samples
    ...
    def cluster(self, new_centers): 
        ...
        while precision_incr > 0:  # cost_decr > 0:
            ...
            self.furthest_outlier_ind = self._assign_positive_samples(pos_dist_mat, neg_dist_mat)
                    
            # ----------------------------------------------------------------------
            #  assign false positives (in fact negatives) based on threshold
            # ----------------------------------------------------------------------
